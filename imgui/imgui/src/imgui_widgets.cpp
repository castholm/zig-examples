...
void ImGui::LabelTextV(const char *label, const char *fmt, va_list args) {
    ...
    const ImRect value_bb(
        pos,
        pos + ImVec2(w, value_size.y + style.FramePadding.y * 2));
...
bool ImGui::DataTypeApplyFromText(const char *buf,
    ...
    if (data_type == ImGuiDataType_Float || data_type == ImGuiDataType_Double)
        ...
        format =
            ImParseFormatSanitizeForScanning(format,
                                             format_sanitized,
...
TYPE ImGui::RoundScalarWithFormatT(const char *format,
    ...
    ImParseFormatSanitizeForPrinting(fmt_start,
                                     fmt_sanitized,
    ...
    if (g.ActiveId == id) {
        ...
        if (select_all) {
            ...
            if ((multiclick_count % 2) == 0) {
                ...
                const bool is_bol =
                    (state->Stb->cursor == 0) ||
                    ImStb::STB_TEXTEDIT_GETCHAR(state,
                                                state->Stb->cursor - 1) == '\n';
                ...
                state->Stb->cursor =
                    ImStb::STB_TEXTEDIT_MOVEWORDRIGHT_MAC(state,
                                                          state->Stb->cursor);
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id) {
        ...
        const bool is_redo =
            (Shortcut(ImGuiMod_Ctrl | ImGuiKey_Y, f_repeat, id) ||
             Shortcut(ImGuiMod_Ctrl | ImGuiMod_Shift | ImGuiKey_Z,
                      f_repeat,
                      id)) &&
            !is_readonly && is_undoable;
...

        {
            ...
            cursor_offset.x =
                InputTextCalcTextSize(&g,
                                      ImStrbol(cursor_ptr, text_begin),
                                      cursor_ptr)
                    .x;
            ...
            if (selmin_line_no >= 0) {
                select_start_offset.x =
                    InputTextCalcTextSize(&g,
                                          ImStrbol(selmin_ptr, text_begin),
                                          selmin_ptr)
                        .x;
        ...
        if (render_selection) {
            ...
            for (const char *p = text_selected_begin; p < text_selected_end;) {
                ...
                if (rect_pos.y < clip_rect.y) {
                    ...
                    ImVec2 rect_size = InputTextCalcTextSize(&g,
                                                             p,
                                                             text_selected_end,
                                                             &p,
                                                             NULL,
        ...
        if (is_multiline ||
            ...
            const ImVec2 draw_scroll =
                /*state ? ImVec2(state->Scroll.x, 0.0f) :*/ ImVec2(
                    0.0f,
                    0.0f); // Preserve scroll when inactive?
            ImU32 col = GetColorU32(is_displaying_hint ? ImGuiCol_TextDisabled
                                                       : ImGuiCol_Text);
            draw_window->DrawList->AddText(g.Font,
                                           g.FontSize,
...
static void ImGuiSelectionBasicStorage_BatchSetItemSelected(
    ...
    ImGuiStoragePair *it = ImLowerBound(storage->Data.Data,
    ...
    if (selected && !is_contained)
        storage->Data.push_back(ImGuiStoragePair(
            id,
            selection_order)); // Push unsorted at end of vector, will be sorted
            ...
            if (req.Selected) {
                ...
                for (int idx = 0; idx < ms_io->ItemsCount;
                    ...
                    ImGuiSelectionBasicStorage_BatchSetItemSelected(
                        ...
                        size_before_amends,
                        _SelectionOrder);
                ImGuiSelectionBasicStorage_BatchFinish(this,
                                                       req.Selected,
                                                       size_before_amends);
            }
        ...
        if (tab_contents_visible &&
            ...
            if (style.TabRounding > 0.0f) {
                ...
                display_draw_list->PathArcToFast(
                    tl + ImVec2(+rounding, +rounding),
                    rounding,
                    7,
                    9);
                display_draw_list->PathArcToFast(
                    tr + ImVec2(-rounding, +rounding),
                    rounding,
                    9,
                    11);
