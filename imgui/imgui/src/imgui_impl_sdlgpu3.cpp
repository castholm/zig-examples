...
static ImGui_ImplSDLGPU3_Data *ImGui_ImplSDLGPU3_GetBackendData() {
    return ImGui::GetCurrentContext() ? (ImGui_ImplSDLGPU3_Data *)ImGui::GetIO()
...
static void CreateOrResizeBuffer(SDL_GPUBuffer **buffer,
                                 uint32_t *old_size,
                                 uint32_t new_size,
    ...
    *old_size = new_size;
    IM_ASSERT(*buffer != nullptr && "Failed to create GPU Buffer, call "
...
void Imgui_ImplSDLGPU3_PrepareDrawData(ImDrawData *draw_data,
    ...
    IM_ASSERT(vertex_transferbuffer != nullptr &&
              "Failed to create the vertex transfer buffer, call "
              "SDL_GetError() for more information");
    ...
    ImDrawVert *vtx_dst =
        (ImDrawVert *)SDL_MapGPUTransferBuffer(v->Device,
                                               vertex_transferbuffer,
                                               true);
    ImDrawIdx *idx_dst =
        (ImDrawIdx *)SDL_MapGPUTransferBuffer(v->Device,
                                              index_transferbuffer,
                                              true);
...
void ImGui_ImplSDLGPU3_RenderDrawData(ImDrawData *draw_data,
    ...
    ImGui_ImplSDLGPU3_SetupRenderState(draw_data,
                                       pipeline,
                                       command_buffer,
                                       render_pass,
                                       fd,
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++) {
            ...
            if (pcmd->UserCallback != nullptr) {
                ...
                SDL_BindGPUFragmentSamplers(
                    render_pass,
                    0,
                    (SDL_GPUTextureSamplerBinding *)pcmd->GetTexID(),
                    1);
...
void ImGui_ImplSDLGPU3_CreateFontsTexture() {
    ...
    // Create all the upload structures and upload:
    {
        ...
        IM_ASSERT(transferbuffer != nullptr &&
                  "Failed to create font transfer buffer, call SDL_GetError() "
                  "for more information");
