...
// Obsolete names
    ...
    ImGuiChildFlags_Border =
        ImGuiChildFlags_Borders, // Renamed in 1.91.1 (August
...
enum ImGuiInputTextFlags_ {
    ...
    ImGuiInputTextFlags_ParseEmptyRefVal =
        1
        << 13, // InputFloat(), InputInt(), InputScalar() etc. only: parse empty
    ...
    ImGuiInputTextFlags_CallbackResize =
        1
        << 22, // Callback on buffer capacity changes request (beyond 'buf_size'
...
struct ImGuiSelectionBasicStorage {
    ...
    void *
        UserData; // = NULL   // User data for use by adapter function        //
...
struct ImDrawList {
    ...
    IMGUI_API void
    AddBezierCubic(const ImVec2 &p1,
                   const ImVec2 &p2,
                   const ImVec2 &p3,
                   const ImVec2 &p4,
                   ImU32 col,
...
struct ImFontAtlas {
    ...
    IMGUI_API void
    GetTexDataAsAlpha8(unsigned char **out_pixels,
                       int *out_width,
                       int *out_height,
                       int *out_bytes_per_pixel = NULL); // 1 byte per-pixel
    IMGUI_API void
    GetTexDataAsRGBA32(unsigned char **out_pixels,
                       int *out_width,
                       int *out_height,
...
struct ImFont {
    ...
    ImVector<float>
        IndexAdvanceX;      // 12-16 // out // Sparse. Glyphs->AdvanceX in
    ...
    const ImFontConfig
        *ConfigData; // 4-8   // in  // Pointer within
                     // ContainerAtlas->ConfigData to ConfigDataCount instances
    ...
    ImWchar
        EllipsisChar; // 2-4   // out // Character used for ellipsis rendering
                      // ('...').
...
// github.com/ocornut/imgui/issues/3361 for details.
// IMGUI_API bool      DragScalar(const char* label, ImGuiDataType data_type,
// void* p_data, float v_speed, const void* p_min, const void* p_max, const
// char* format, float power = 1.0f) // OBSOLETED in 1.78 (from June 2020)
// IMGUI_API bool      DragScalarN(const char* label, ImGuiDataType data_type,
// void* p_data, int components, float v_speed, const void* p_min, const void*
// p_max, const char* format, float power = 1.0f); // OBSOLETED in 1.78 (from
// June 2020) IMGUI_API bool      SliderScalar(const char* label, ImGuiDataType
// data_type, void* p_data, const void* p_min, const void* p_max, const char*
// format, float power = 1.0f); // OBSOLETED in 1.78 (from June 2020) IMGUI_API
// bool      SliderScalarN(const char* label, ImGuiDataType data_type, void*
// p_data, int components, const void* p_min, const void* p_max, const char*
// format, float power = 1.0f); // OBSOLETED in 1.78 (from June 2020) static
// inline bool  DragFloat(const char* label, float* v, float v_speed, float
// v_min, float v_max, const char* format, float power = 1.0f)    { return
// DragScalar(label, ImGuiDataType_Float, v, v_speed, &v_min, &v_max, format,
// power); }     // OBSOLETED in 1.78 (from June 2020) static inline bool
// DragFloat2(const char* label, float v[2], float v_speed, float v_min, float
...
// OBSOLETED in 1.78 (from June 2020) static inline bool  DragFloat3(const char*
// label, float v[3], float v_speed, float v_min, float v_max, const char*
// format, float power = 1.0f) { return DragScalarN(label, ImGuiDataType_Float,
// v, 3, v_speed, &v_min, &v_max, format, power); } // OBSOLETED in 1.78 (from
// June 2020) static inline bool  DragFloat4(const char* label, float v[4],
...
// SliderScalarN(label, ImGuiDataType_Float, v, 3, &v_min, &v_max, format,
// power); }        // OBSOLETED in 1.78 (from June 2020) static inline bool
...
// the first place. typedef ImGuiKeyChord ImGuiModFlags;      // == int. We
// generally use ImGuiKeyChord to mean "a ImGuiKey or-ed with any number of
// ImGuiMod_XXX value", so you may store mods in there. enum ImGuiModFlags_ {
